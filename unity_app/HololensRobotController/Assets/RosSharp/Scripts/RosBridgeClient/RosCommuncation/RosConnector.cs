...
namespace RosSharp.RosBridgeClient
{
    public class RosConnector : MonoBehaviour
    {
        ...
        private string RosBridgeServerUrl = Config.RosBridgeServerUrl;

        private ManualResetEvent isConnected = new ManualResetEvent(false);

        // TODO: Remove this function since it is not effective, find a way to take IP address as a configuration
        public void SetRosBridgeServerUrl(string serverIPAddress)
        {
            RosBridgeServerUrl = serverIPAddress;
        }
        ...
        private void ConnectAndWait()
        {
            ...
            if (!isConnected.WaitOne(Timeout * 1000))
                System.Diagnostics.Debug.WriteLine("Failed to connect to RosBridge at: " + RosBridgeServerUrl);
            else
                System.Diagnostics.Debug.WriteLine("Successfully connected to RosBridge at: " + RosBridgeServerUrl);
        }
